...
- Consolidated the repository-organization work from `main` into the active
  non-production remediation branch while preserving the approved owner-content
  restoration and every Batch A release control.
- Removed the superseded August pre-sitemap Markdown set, the obsolete 10-county
  planning dataset, the completed privacy/terms benchmark, the inaccurate
  location-deployment report, and the point-in-time file inventory. All remain
  recoverable from Git history.
- Updated active SEO and payer registers so they match the current 85-route
  sitemap, 65-location inventory, 118 redirects, owner attestation, and payer
  evidence register.
- Preserved current governance, management-layer, keyword, competitor, review,
  resource, security, and release documentation.
...
- Remote deletion requires a GitHub ref-delete capability; the connected API in
  ...
  push cannot perform the operation.
